                                ...
                                for (Player plBoss : listBoss) {
                                    if (plBoss != null) {
                                        if (pl.playerTask.taskMain.id != 19 && (plBoss.id == BossID.KUKU || plBoss.id == BossID.MAP_DAU_DINH || plBoss.id == BossID.RAMBO)) {
                                            ...
                                            Service.getInstance().sendThongBaoOK(pl, " Map Trong thời gian hỗ trợ\nKhông thể vào khu vực này");
                                            return;
                                        }
                                        ...
                                        if (pl.playerTask.taskMain.id != 22 && (plBoss.id == BossID.ANDROID_19 || plBoss.id == BossID.DR_KORE)) {

                                            Service.getInstance().sendThongBaoOK(pl, " Map Trong thời gian hỗ trợ\nKhông thể vào khu vực này");
                                            return;
                                        }
//                                        if (pl.playerTask.taskMain.id != 26 && (plBoss.id == BossID.ANDROID_13 || plBoss.id == BossID.ANDROID_14 || plBoss.id == BossID.ANDROID_15)) {
//
//                                            Service.getInstance().sendThongBaoOK(pl, " Map Trong thời gian hỗ trợ\nKhông thể vào khu vực này");
//                                            return;
//                                        }
                                        if (pl.playerTask.taskMain.id != 23 && (plBoss.id == BossID.PIC || plBoss.id == BossID.POC || plBoss.id == BossID.KING_KONG)) {
    ...
    public Zone checkMapCanJoin(Player player, Zone zoneJoin) {
        ...
        if ( player != null && zoneJoin != null && player.isPl() && !player.isAdmin()) {
            switch (zoneJoin.map.mapId) {
                ...
                case 168:
                case 169:
                    if (player.getSession().player.nPoint.power <= 120_000_000_000L) {
                        Service.getInstance().sendThongBao(player, "Yêu cầu sức mạnh trên 120 tỷ");
                        zoneJoin = null;
                        if (TaskService.gI().getIdTask(player) < ConstTask.TASK_23_0) {
                            Service.gI().sendThongBao(player, "Vui lòng hoàn thành nhiệm vụ Fide trước khi tới đây!");
                            return null;
                        }
                    }
                    break;
